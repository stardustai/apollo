 ...
 *****************************************************************************/
#include "modules/perception/lidar/tools/stardust_exporter/stardust_msg_exporter.h"
...
StardustMsgExporter::StardustMsgExporter(std::shared_ptr<apollo::cyber::Node> node,
...
void StardustMsgExporter::ImageMessageHandler(
...
void StardustMsgExporter::PointCloudMessageHandler(
...
bool StardustMsgExporter::SavePointCloud(
...
bool StardustMsgExporter::SaveImage(const unsigned char* color_image,
...
bool StardustMsgExporter::QuerySensorToWorldPose(double timestamp,
...
bool StardustMsgExporter::QueryPose(double timestamp, const std::string& frame_id,
                            const std::string& child_frame_id,
                            Eigen::Matrix4d* pose) {
...
bool StardustMsgExporter::SavePose(const Eigen::Matrix4d& pose, double timestamp,
                           const std::string& folder) {
...
bool StardustMsgExporter::IsStereoCamera(const std::string& channel) {
  const std::vector<std::string> strs = absl::StrSplit(channel, '/');
  return strs.size() > 2 && strs[2] == "smartereye";
}

bool StardustMsgExporter::IsCamera(const std::string& channel) {
  const std::vector<std::string> strs = absl::StrSplit(channel, '/');
  return strs.size() > 1 && strs[1] == "camera";
}

bool StardustMsgExporter::IsLidar(const std::string& channel) {
  const std::vector<std::string> strs = absl::StrSplit(channel, '/');
  return strs.size() > 0 && strs.back() == "PointCloud2";
}

std::string StardustMsgExporter::TransformChannelToFolder(const std::string& channel) {
  const std::vector<std::string> strs = absl::StrSplit(channel, '/');
  std::string target = "";
  for (auto& str : strs) {
    target += str + "_";
